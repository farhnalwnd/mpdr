...
<body>
    <div class="container">
        <div class="header">
            ...
            @if(isset($form_no))
            <div class="fs-16 mt-5">Pre-MPDR No Registration : {{ $form_no }}</div>
            @endif
        </div>
        <form action="{{ route('prempdr.comment', ['form_no' => $form_no, 'approver_nik' => $approver_nik, 'status' => $status, 'token' => $token]) }}" method="POST" style="margin-top: 20px;">
            @csrf
